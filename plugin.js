import { stripInlineNotes } from '../../../utils';
/**
 ...
 *
 * This is a copy of the paste plugin from TinyMCE with the PR https://github.com/tinymce/tinymce/pull/5197 applied.
 * Once that PR has been merged (if ever), this plugin can be removed and TinyMCE updated.
 */
(function (domGlobals) {
  ...
  var pasteHtml$1 = function (editor, html, internalFlag) {
    var internal = internalFlag || InternalHtml.isMarked(html);
  ...
  /**
   * Strip out any inline-note spans
   * @param editor
   * @returns object data
   */
  const stripInlineNotesFromEditor = editor => {
    const data = getData(editor);
    return { ...data, html: stripInlineNotes(data.html) };
  };
  ...
  var copy = function (editor) {
    return function (evt) {
      if (hasSelectedContent(editor)) {
        setClipboardData(
          evt,
          stripInlineNotesFromEditor(editor),
          fallback(editor),
          noop
        );
      }
  ...
  function Plugin() {
    global.add('gmpaste', function (editor) {
